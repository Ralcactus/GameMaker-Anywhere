yyp = get_open_filename("GameMaker Project|*.yyp", "");

//error and restart if you don't select a project
if (yyp == ""){
...
//get the first room
first_room_name = yyp_json.RoomOrderNodes[0].roomId.name;
first_room_path = yyp_json.RoomOrderNodes[0].roomId.path;
//get all resources
all_resources = [];
all_rooms = [];
all_sprites = [];
...
for (var i = 0; i < array_length(yyp_json.resources); i++)
{
	...
	if (yyfile.resourceType == "GMRoom")
		IsRoom = true;
	if (yyfile.resourceType == "GMSprite")
		IsSprite = true;
	if (yyfile.resourceType == "GMObject")
		IsObject = true;
	...
	if (IsSprite){
		...
		//make a folder for the sprite
		directory_create(sprites_root + yyfile.name + "\\");

		//add sprite info for data.win
		...
		for (var f = 0; f < array_length(yyfile.frames); f++)
		{
			...
			file_copy(filename_dir(yyp) + "\\sprites\\" + yyfile.name + "\\" + frame_name + ".png", sprites_root + yyfile.name + "\\" + frame_name + ".png");
	...
	if (IsObject){
		...
		if (variable_struct_exists(yyfile, "spriteId"))
		{
			...
			// spriteId can be a struct {name,path}, or null/undefined
...
//convert to go back into a json (data.win)
var export_json = {
    FirstRoom: {
